...
1. 在装有 **Unity 2022.3.62f3c1 + WebGL Build Support 模块**的机器上打开本地工程 `C:\Users\李\lianxi`（不在仓库内）
2. 菜单 **Build → S5 → 构建 WebGL（数字孪生）**（脚本：`Assets/Editor/S5WebGLBuilder.cs`），
...
> 本仓库已附带构建好的产物（2026-09-01，WebGL 约 106MB + 视频 59MB），clone 后直接启动前端即可看到孪生场景。

> 注意：本文档旧版 API 示例写的是 `/api/v1/s5/`，实际服务统一使用 `/api/s5/`（按 S5 手册"不要自创前缀"约束），以 `twin-csharp/` 与 `backend/` 为准。
