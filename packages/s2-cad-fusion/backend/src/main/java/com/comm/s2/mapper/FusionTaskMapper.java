...
public interface FusionTaskMapper extends BaseMapper<FusionTask> {
    ...
    @Select("SELECT * FROM s2_fusion_task WHERE project_id = #{projectId} ORDER BY create_time DESC")
    List<FusionTask> findByProjectId(@Param("projectId") Long projectId);

    @Select("SELECT * FROM s2_fusion_task WHERE created_by = #{userId} ORDER BY create_time DESC")
    List<FusionTask> findByUserId(@Param("userId") Long userId);

    @Select("SELECT * FROM s2_fusion_task WHERE status = #{status}")
