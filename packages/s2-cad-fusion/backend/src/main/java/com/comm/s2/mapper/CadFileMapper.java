...
public interface CadFileMapper extends BaseMapper<CadFile> {
    ...
    @Select("SELECT * FROM s2_cad_file WHERE project_id = #{projectId} ORDER BY create_time DESC")
    List<CadFile> findByProjectId(@Param("projectId") Long projectId);

    @Select("SELECT * FROM s2_cad_file WHERE uploaded_by = #{userId} ORDER BY create_time DESC")
    List<CadFile> findByUserId(@Param("userId") Long userId);

    @Select("SELECT * FROM s2_cad_file WHERE parse_status = #{status}")
