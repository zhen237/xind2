-- S2 CAD Fusion 数据库表结构
-- 多源异构数据融合服务

-- CAD文件表
CREATE TABLE IF NOT EXISTS s2_cad_file (
...
-- 坐标系配置表
CREATE TABLE IF NOT EXISTS s2_coordinate_system (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
...
-- 融合任务表
CREATE TABLE IF NOT EXISTS s2_fusion_task (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
...
-- GIS要素表
CREATE TABLE IF NOT EXISTS s2_gis_feature (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    ...
    coordinate_x DECIMAL(12, 8) COMMENT 'X坐标(经度，要素中心点)',
    coordinate_y DECIMAL(12, 8) COMMENT 'Y坐标(纬度，要素中心点)',
    coordinate_z DECIMAL(12, 8) DEFAULT 0 COMMENT 'Z坐标(高度)',
    geometry_json TEXT COMMENT '完整几何GeoJSON(含全部顶点，目标坐标系)',
...
-- 初始化预置坐标系数据
INSERT INTO s2_coordinate_system (epsg_code, name, type, projection, datum, is_preset, description) VALUES
