...
```
packages/s2-cad-fusion/
├── backend/
│   ├── src/main/java/com/comm/s2/
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/s2/cad/cad-files/upload` | 上传 CAD 文件 (DWG/DXF) |
| GET | `/api/s2/cad/cad-files/{id}` | 获取文件信息 |
| GET | `/api/s2/cad/cad-files` | 获取文件列表 |
| DELETE | `/api/s2/cad/cad-files/{id}` | 删除文件 |
| POST | `/api/s2/cad/cad-files/{id}/parse` | 解析 DXF 文件 |
| GET | `/api/s2/cad/cad-files/{id}/content` | 下载原文件 |

### 坐标转换

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/s2/cad/coordinate/transform` | 单点坐标转换 |
| POST | `/api/s2/cad/coordinate/batch-transform` | 批量坐标转换 |
| POST | `/api/s2/cad/coordinate/wgs84-to-cgcs2000` | WGS84→CGCS2000 |
| POST | `/api/s2/cad/coordinate/beijing54-to-wgs84` | Beijing54→WGS84 |
| GET | `/api/s2/cad/coordinate/supported-systems` | 获取支持的坐标系 |

### 数据融合

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/s2/cad/fusion/tasks` | 创建融合任务 |
| GET | `/api/s2/cad/fusion/tasks/{id}` | 获取任务状态 |
| POST | `/api/s2/cad/fusion/tasks/{id}/execute` | 执行融合任务 |
| POST | `/api/s2/cad/fusion/auto-fuse` | 一键融合（创建+执行） |
| DELETE | `/api/s2/cad/fusion/tasks/{id}` | 删除任务 |
| GET | `/api/s2/cad/fusion/tasks/{id}/geojson` | 获取融合结果(GeoJSON) |
...
```bash
# 上传文件
POST /api/s2/cad/cad-files/upload
...
```bash
# WGS84 转 CGCS2000
POST /api/s2/cad/coordinate/wgs84-to-cgcs2000?lon=114.39&lat=30.51

# 通用转换
POST /api/s2/cad/coordinate/transform
{
  "sourceX": 114.39,
  "sourceY": 30.51,
  "sourceZ": 0,
...
```bash
POST /api/s2/cad/fusion/auto-fuse
{
  "taskName": "东湖基站设计融合",
  "projectId": 101,
  "sourceFileId": 1,
...
```bash
GET /api/s2/cad/fusion/tasks/{taskId}/geojson
```
...
mvn clean package
java -jar target/s2-cad-fusion-1.0.0.jar
```
